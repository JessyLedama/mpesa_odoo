...
class MpesaTransaction(models.Model):
    ...
    def name_get(self):
        result = []
        for record in self:
            name = record.transaction_id or f'TXN-{record.id}'
    ...
    def create_from_callback(self, callback_data, transaction_type='c2b'):
        ...
        _logger.info(f'Creating transaction from callback: {callback_data}')
    ...
    def _create_c2b_transaction(self, data):
        ...
        if not config:
            _logger.error(f"No config found for shortcode: {data.get('BusinessShortCode')}")
            return False
        ...
        if trans_time:
            try:
                transaction_time = fields.Datetime.to_datetime(
                    f'{trans_time[:4]}-{trans_time[4:6]}-{trans_time[6:8]} '
                    f'{trans_time[8:10]}:{trans_time[10:12]}:{trans_time[12:14]}'
                )
            except (ValueError, IndexError):
                _logger.warning(f'Could not parse transaction time: {trans_time}')
    ...
    def _update_stk_transaction(self, data):
        ...
        if not transaction:
            _logger.error(f'No transaction found for CheckoutRequestID: {checkout_request_id}')
            return False
    ...
    def _reconcile_with_invoice(self, invoice):
        ...
        try:
            ...
            payment_vals = {
                ...
                'ref': f'M-Pesa: {self.transaction_id}',
            ...
            _logger.info(f'Successfully reconciled M-Pesa transaction {self.transaction_id} with invoice {invoice.name}')
            return True

        except Exception as e:
            _logger.error(f'Failed to reconcile transaction {self.transaction_id}: {str(e)}')
            return False
    ...
    def _cron_check_pending_transactions(self):
        ...
        for transaction in pending_stk:
            try:
                ...
                _logger.warning(f'Error checking STK status for {transaction.id}: {str(e)}')
        ...
        for transaction in unreconciled:
            try:
                ...
                _logger.warning(f'Error reconciling transaction {transaction.id}: {str(e)}')
